const initRLS=()=>{
  ...
  if(header){
    ...
    if(inner){
      ...
      if(!switcher){switcher=document.createElement('select');switcher.className='language-switcher header-language-switcher';switcher.setAttribute('aria-label',lang==='fa'?'انتخاب زبان':lang==='en'?'Language':'Язык');inner.appendChild(switcher);}
